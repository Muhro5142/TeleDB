...
def get_command_buttons():
    commands = [
        {'name': 'Новый проект✔', 'callback_data': '/new_project'},
        {'name': 'Все проекты🧰', 'callback_data': '/projects'},
        {'name': 'Изменить проект🛠', 'callback_data': '/update_projects'},
        {'name': 'Навыки📥', 'callback_data': '/skills'},
        {'name': 'Удалить проект❌', 'callback_data': '/delete'},
    ]
    markup = InlineKeyboardMarkup()
    for command in commands:
        markup.add(InlineKeyboardButton(command['name'], callback_data=command['callback_data']))
    return markup
...
Также ты можешь ввести имя проекта и узнать информацию о нем!""", reply_markup=get_command_buttons())

@bot.callback_query_handler(func=lambda call: call.data in ['/new_project', '/projects', '/update_projects', '/skills', '/delete'])
def handle_commands(call):
    message = call.message
    if call.data == '/new_project':
        addtask_command(message)
    elif call.data == '/projects':
        get_projects(message)
    elif call.data == '/update_projects':
        update_project(message)
    elif call.data == '/skills':
        skill_handler(message)
